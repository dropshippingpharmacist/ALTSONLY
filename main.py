...
================================================================================
MULTI-TIMEFRAME VERSION - SCANS: 1h, 2h, 3h, 4h, 6h, 8h, 12h, 1d
================================================================================
"""
...
# =============================================================================
# MULTI-TIMEFRAME CONFIGURATION (ADDED)
# =============================================================================
# All higher timeframes available on Bitget
HIGH_TIMEFRAMES = ["1h", "2h", "3h", "4h", "6h", "8h", "12h", "1d"]

# Map timeframe to API string
TIMEFRAME_MAP = {
...
class ValidatedRange:
    ...
    timeframe: str = "4h"  # ADDED: track which timeframe this range came from
...
# =============================================================================
# ENHANCED TCT ANALYZER (MAIN ENGINE) - MODIFIED FOR MULTI-TIMEFRAME
# =============================================================================
class EnhancedTCTAnalyzer:
    """COMPLETE TCT ANALYSIS WITH ALL ADVANCED CONCEPTS - MULTI-TIMEFRAME"""
    ...
    def analyze(self, symbol: str, htf_data: Dict[str, pd.DataFrame], 
        ...
        """Analyze multiple higher timeframes and return the best signal"""
        
        best_signal = None
        best_score = -1
        
        # Loop through all higher timeframes
        for tf in HIGH_TIMEFRAMES:
            ...
            # Step 1: Detect valid range on this timeframe
            ...
            obs = self.sd_detector.detect_order_blocks(df_htf, valid_range)
            ...
            levels = self.level_analyzer.analyze(df_htf, df_1h, df_15m)
            ...
            if len(high_devs) >= 2 and valid_entry_short and ltf_confirmed_short:
                ...
                if self._check_tap_spacing(tap1, tap2, tap3) and tap3 > tap2:
                    ...
                    if rr >= MIN_RR:
                        ...
                        has_test, test_price = self._detect_test_phase(df_htf, valid_range, "short")
                        
                        # EHP Detection
                        is_ehp = self._detect_ehp(df_htf, df_1h, valid_range, "short")
                        ...
                        if confidence >= MIN_CONFIDENCE and duration_ok:
                            ...
                            if grade in [SetupGrade.A_PLUS, SetupGrade.A]:
                                ...
                                signal = EnhancedTCTSignal(
                                    ...
                                    reason=f"[{tf}] {model.value} | {ltf_info} | Grade:{grade.value}",
                                ...
                                # Score this signal
                                signal_score = confidence * (1 + rr/10) + (0.1 if is_ehp else 0)
            ...
            if len(low_devs) >= 2 and valid_entry_long and ltf_confirmed_long:
                ...
                if self._check_tap_spacing(tap1, tap2, tap3) and tap3 < tap2:
                    ...
                    if rr >= MIN_RR:
                        ...
                        has_test, test_price = self._detect_test_phase(df_htf, valid_range, "long")
                        is_ehp = self._detect_ehp(df_htf, df_1h, valid_range, "long")
            ...
            if len(high_devs) == 1 and valid_entry_short and ltf_confirmed_short:
                ...
                extreme_liq = self._find_extreme_liquidity(df_htf, "short")
                            ...
                            if confidence >= MIN_CONFIDENCE and duration_ok:
                                ...
                                if grade in [SetupGrade.A_PLUS, SetupGrade.A]:
                                    ...
                                    signal = EnhancedTCTSignal(
                                        ...
                                        confidence=confidence,
                                        position_size=self._calc_position(entry, stop, confidence),
                                        reason=f"[{tf}] M2D | extreme {extreme_type} | {ltf_info} | Grade:{grade.value}",
                                        valid_range=valid_range, tap1=tap1, tap2=tap2, tap3=tap3,
                                        qrz=qrz, levels=levels, context_rank=context,
                                    ...
                                    signal_score = confidence * (1 + rr/10)
                                    if signal_score > best_score:
                                        best_score = signal_score
                                        best_signal = signal
            ...
            if len(low_devs) == 1 and valid_entry_long and ltf_confirmed_long:
                ...
                extreme_liq = self._find_extreme_liquidity(df_htf, "long")
                            ...
                            if confidence >= MIN_CONFIDENCE and duration_ok:
                                ...
                                if grade in [SetupGrade.A_PLUS, SetupGrade.A]:
                                    ...
                                    signal = EnhancedTCTSignal(
                                        ...
                                        confidence=confidence,
                                        position_size=self._calc_position(entry, stop, confidence),
                                        reason=f"[{tf}] M2A | extreme {extreme_type} | {ltf_info} | Grade:{grade.value}",
                                        valid_range=valid_range, tap1=tap1, tap2=tap2, tap3=tap3,
                                        qrz=qrz, levels=levels, context_rank=context,
                                        setup_grade=grade, breaker_quality=ms.breaker_quality,
                                        range_duration_hours=valid_range.duration_hours
                            ...
                            if confidence >= MIN_CONFIDENCE:
                                ...
                                if grade in [SetupGrade.A_PLUS, SetupGrade.A]:
                                    signal = EnhancedTCTSignal(
                                        ...
                                        confidence=confidence,
                                        position_size=self._calc_position(entry, stop, confidence),
                                        reason=f"[{tf}] PO3 Bullish | LTF({best_tf_long}) | Grade:{grade.value}",
                            ...
                            if confidence >= MIN_CONFIDENCE:
                                ...
                                if grade in [SetupGrade.A_PLUS, SetupGrade.A]:
                                    signal = EnhancedTCTSignal(
                                        ...
                                        confidence=confidence,
                                        position_size=self._calc_position(entry, stop, confidence),
                                        reason=f"[{tf}] PO3 Bearish | LTF({best_tf_short}) | Grade:{grade.value}",
                ...
                if rr >= MIN_RR:
                    ...
                    if confidence >= MIN_CONFIDENCE:
                        signal = EnhancedTCTSignal(
                            ...
                            confidence=confidence,
                            position_size=self._calc_position(entry, stop, confidence),
                            reason=f"[{tf}] Two-Tap Short | RTZ={rtz:.2f}",
                ...
                if rr >= MIN_RR:
                    ...
                    if confidence >= MIN_CONFIDENCE:
                        signal = EnhancedTCTSignal(
                            ...
                            confidence=confidence,
                            position_size=self._calc_position(entry, stop, confidence),
                            reason=f"[{tf}] Two-Tap Long | RTZ={rtz:.2f}",
...
# =============================================================================
# DATA FETCHER - MODIFIED FOR MULTI-TIMEFRAME
# =============================================================================
class DataFetcher:
    def __init__(self):
    ...
    def get_all_altcoins(self) -> List[str]:
        ...
        log.info("📊 Fetching top 100 altcoins by volume...")
        
        try:
            markets = self.exchange.load_markets()
            usdt_pairs = [sym for sym, m in markets.items() 
                         if sym.endswith('/USDT') and m.get('active')]
            
            exclude = ['USDC', 'BUSD', 'DAI', 'TUSD', 'USDP', 'FDUSD', 'USTC', 'USDD',
                      'USDE', 'WBTC', 'WETH']
            ...
            top_100 = [p[0] for p in pair_volumes[:MAX_PAIRS_TO_SCAN]]
            
            self._top_coins_cache = top_100
            self._cache_time = current_time
            
            log.info(f"✅ Loaded {len(top_100)} coins with >${MIN_VOLUME_USD:,.0f} volume")
            return top_100
    ...
    def fetch_ohlcv(self, symbol: str, timeframe: str = "4h", limit: int = 200) -> pd.DataFrame:
        ...
        try:
            # Use the timeframe map for all timeframes
...
# =============================================================================
# TELEGRAM NOTIFIER - MODIFIED TO SHOW TIMEFRAME
# =============================================================================
class TelegramNotifier:
    ...
    def send_signal(self, s: EnhancedTCTSignal) -> bool:
        ...
        # Extract LTF and timeframe from reason string
        ltf = "Unknown"
        tf_display = s.valid_range.timeframe if s.valid_range else "4h"
        if 'LTF(' in s.reason:
            import re
        ...
        msg = f"""{direction_text} {s.symbol} [{tf_display}]
...
# =============================================================================
# MAIN SCANNER - MODIFIED FOR MULTI-TIMEFRAME
# =============================================================================
class EnhancedTCTScanner:
    ...
    def _banner(self):
        print("\n" + "=" * 80)
        print("  ENHANCED PURE TCT BOT - MULTI-TIMEFRAME VERSION")
        print("=" * 80)
        print("  ORIGINAL LECTURES 1-8: 100% PRESERVED")
        print("  ADVANCED PDF CONCEPTS: FULLY INTEGRATED")
        print("=" * 80)
        print(f"  📊 HIGH TIMEFRAMES SCANNED: {', '.join(HIGH_TIMEFRAMES)}")
    ...
    def _fetch_all_timeframes(self, symbol: str) -> Tuple[Dict[str, pd.DataFrame], Dict[str, pd.DataFrame]]:
        """Fetch data for all higher timeframes and lower timeframes"""
        htf_data = {}
        ltf_data = {}
        
        # Fetch all higher timeframes
        for tf in HIGH_TIMEFRAMES:
        ...
        # Fetch lower timeframes for confirmation
        lower_tfs = ['5m', '15m', '30m', '1h']
        for tf in lower_tfs:
            df = self.data.fetch_ohlcv(symbol, tf, 200)
            if len(df) >= 30:
                ltf_data[tf] = df
    ...
    def run_once(self):
        ...
        for symbol in coins:
            ...
            if signal:
                ...
                tf_display = signal.valid_range.timeframe if signal.valid_range else "4h"
        
                print(f"\n  ✅ {label} {signal.symbol} [{tf_display}] {grade_display} {ehp_mark}{test_mark}")
